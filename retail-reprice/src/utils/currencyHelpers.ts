...
export class CurrencyHelpers {
  ...
  private static formatStringForCurrency = function (
    ...
    if (!StringHelpers.isNumeric(costInCents)) {
      console.error(
        new Error(
          `Value (${costInCents}) passed for currency formatting cannot be parsed to a number`
        )
      );
      return '';
    }
    return CurrencyHelpers.formatNumberForCurrency(
      parseInt(costInCents, 10),
      locale,
      currencyCode,
      fractionDigits
    );
  ...
  private static formatMoneyCurrency = function (
    price: Money,
    locale?: string
  ) {
  ...
  static formatForCurrency = (
    ...
    !costInCents
      ...
      ? CurrencyHelpers.formatStringForCurrency(
          costInCents,
          locale,
          currencyCode,
          fractionDigits
      ...
      ? CurrencyHelpers.formatNumberForCurrency(
          costInCents,
          locale,
          currencyCode,
          fractionDigits
